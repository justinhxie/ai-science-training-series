...
test_dset = dataset.shard(num_shards=hvd.size(), index=hvd.rank())
...
def training_step(images, labels):
    ...
    ### Set distributed optimizer / gradient tape
...
for ep in range(args.epochs):
    ...
    for batch, (images, labels) in enumerate(dataset.take(nstep)):
        ...
        acc = training_acc
        ### broadcast model and parameters from rank 0 to the other ranksx
        if (nstep==0 and ep == 0):
    ...
    for batch, (images, labels) in enumerate(test_dset.take(ntest_step)):
        ...
        ### average the metrics 
        test_loss = hvd.allreduce(loss_value, average=True)
        test_acc = hvd.allreduce(acc, average=True)
